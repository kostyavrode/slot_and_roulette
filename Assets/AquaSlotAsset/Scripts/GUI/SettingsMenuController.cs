...
namespace Mkey
{
    public class SettingsMenuController : PopUpsController
    {
        ...
        {
           if (GuiController.Instance)  GuiController.Instance.ShowMessageAbout("", "", () => { Debug.Log("Support"); if (!string.IsNullOrEmpty(SUPPORT_URL)) Application.OpenURL(SUPPORT_URL); }, null, null);
        }
        ...
        public override void RefreshWindow()
        {
           // SetSoundButtVolume(SoundMasterController.Instance.Volume); // not used
            SetMusicButtSprite(SoundMasterController.Instance.MusicOn);
            SetSoundButtSprite(SoundMasterController.Instance.SoundOn);
            if (getCoinsText && SlotPlayer.Instance) getCoinsText.text = "get +" + SlotPlayer.Instance.facebookCoins;
            base.RefreshWindow();
        }
